<x-layout title="Admin - Edit Article | DevSense" description="Edit existing article or documentation guide">
                    ...
                    @foreach(['en', 'ru', 'ua', 'bg'] as $loc)
                        ...
                        <div id="tab-{{ $loc }}" class="tab-pane {{ $loop->first ? 'active' : '' }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="translations[{{ $loc }}][description]" id="description_{{ $loc }}" rows="2" placeholder="SEO meta description..." class="form-input">{{ old("translations.{$loc}.description", $translation?->description) }}</textarea>
                            </div>
