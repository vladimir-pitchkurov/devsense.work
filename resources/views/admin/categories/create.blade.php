<x-layout title="Admin - {{ __('ui.admin.create_category') }} | DevSense" description="Create a new article category">
<div class="admin-container">
    <div class="admin-header">
        <h1 class="admin-title">{{ __('ui.admin.create_category') }}</h1>
        <a href="{{ route('admin.categories.index', ['locale' => app()->getLocale()]) }}" class="admin-btn admin-btn--secondary">
            {{ __('ui.admin.back_to_list') }}
        </a>
    </div>

    <form action="{{ route('admin.categories.store', ['locale' => app()->getLocale()]) }}" method="POST" class="admin-form">
        @csrf
            ...
            <div class="form-sidebar">
                <div class="admin-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required placeholder="e.g. php-version" class="form-input">
                        <p class="form-help">{{ __('ui.admin.slug_help') }}</p>
                    </div>

                    <button type="submit" class="admin-btn admin-btn--primary admin-btn--full">
                        {{ __('ui.admin.create_category_btn') }}
                    </button>
                </div>
            </div>
                    ...
                    @foreach(\App\Http\Middleware\SetLocale::SUPPORTED_LOCALES as $loc)
                        <div id="tab-{{ $loc }}" class="tab-pane {{ $loop->first ? 'active' : '' }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="translations[{{ $loc }}][name]" id="name_{{ $loc }}" value="{{ old("translations.{$loc}.name") }}" required placeholder="{{ __('ui.admin.category_name_placeholder', ['locale' => $loc]) }}" class="form-input">
                            </div>
                        </div>
                    @endforeach
                </div>
