<x-layout title="Admin - Create Category | DevSense" description="Create a new article category">
                ...
                <div class="admin-card">
                    ...
                    @foreach(['en', 'ru', 'ua', 'bg'] as $loc)
                        <div id="tab-{{ $loc }}" class="tab-pane {{ $loop->first ? 'active' : '' }}">
                            ...
                            <div class="form-group">
                                <label for="name_{{ $loc }}" class="form-label">Category Name ({{ strtoupper($loc) }})</label>
                                <input type="text" name="translations[{{ $loc }}][name]" id="name_{{ $loc }}" value="{{ old("translations.{$loc}.name") }}" required placeholder="Category name in {{ $loc }}" class="form-input">
                            </div>
                        </div>
                    @endforeach
                </div>
